...
submit_button_create_server.addEventListener('click', () => {
  ...
  fetch(`/servidores/criarServidor`, {
    ...
    body: JSON.stringify({
      servidores: [{ fk_empresa, nome, pais, mac_address: mac, tipo_cpu, ram, disco }]
    })
  })
    .then(resp => resp.json())
    .then((data) => {
...
function open_modal_edit_server(id, nome, pais, mac, tipo_cpu, ram, disco) {
  ...
  edit_server_modal.querySelector('.pais_input').value = pais;
...
submit_button_edit_server.addEventListener('click', () => {
  ...
  const pais = edit_server_modal.querySelector('.pais_input').value.trim();
  ...
  fetch(`/servidores/atualizarServidor/${id}`, {
    method: 'PUT',
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ nome, pais, mac_address: mac, tipo_cpu, ram, disco })
  })
    .then(resp => resp.json())
    .then(() => {
      ...
      servidores.forEach(s => {
        ...
        if (sessionStorage.getItem("CARGO") === "Analista") {
          div.querySelector('.edit_user_button').addEventListener('click', () =>
            open_modal_edit_server(s.id_servidor || s.id, s.nome, s.pais, s.mac_address, s.tipo_cpu, s.ram, s.disco)
          );
